...
void Graf::zamienGrafNaNieskierowany() {
    ...
    for (i = 0; i < krawedzie; i++) {
        ...
        while (j < krawedzieOdwrotne) {
            ...
            // ZnajdŸ odwrócon¹ krawêdŸ.
            if ((KO[j].wp == K[i].wp && KO[j].wk == K[i].wk) || (KO[j].wp == K[i].wk && KO[j].wk == K[i].wp)) {
                // Je¿eli istnieje krawêdŸ o mniejszej wadze, to zachowaj tê krawêdŸ.
...
void Graf::macierz_Forda(int start) {
    ...
    while (zmiana) {
        ...
        for (int i = 0; i < wierzcholki; i++) {
            for (int j = 0; j < wierzcholki; j++) {
                if (macierzSasiedztwa[i][j] != 0) { // Sprawdzenie czy krawêdŸ istnieje
...
void Graf::lista_FordFulkerson(int zrodlo, int ujscie) {
    ...
    // G³ówna pêtla, wykonywana dopóki istnieje œcie¿ka od Ÿród³a do ujœcia.
    while (true) {
